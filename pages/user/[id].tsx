...
const UserId = ({ User, Receiver }: UserIdProps) => {
    ...
    const handleSendClick = useCallback(async (e: KeyboardEvent) => {
            mutate({ chat: Chat?.chat.concat({ message, profile: User.image, username: User.name, userId: User.id, receiverProfile: Receiver.profile, receiverUserId: Receiver.userId, receiverUsername: Receiver.username, createdAt: new Date().toISOString()}) } as { chat: ChatPostType[] }, { revalidate: false })
            setIsSend(true)
            const res = await fetch(BASEURL + "/api/chat", {
                method: "POST",
    ...
    return (
        <main className='w-screen h-screen overflow-y-scroll bg-stone-900 pb-20'>
            <header className='w-screen h-[12vh] bg-stone-800 flex justify-around items-center gap-3'>
                <div className='flex justify-start gap-2 items-center'>
                    <Images profile={Receiver.profile} height={55} width={55} />
                    <span>
                        <p className='text-orange-50 font-[500] tracking-wider text-lg'>{Receiver.username}</p>
                        <p className='text-[.8rem] text-green-400'>Online</p>
                    </span>
                </div>
            ...
            <section className='flex flex-col place-items-around justify-center gap-3 w-screen overflow-y-scroll pt-3 px-2'>
                {!Chat?.chat.length && <p className='text-stone-300 text-center text-sm tracking-wider mt-8'>Start Chat 😃</p>}
                {Chat?.chat.map((chat, i) => (
                    <>
                        <nav className={` rtl p-2  h-max  ${chat.username === User.name ? "place-self-end rounded-bl-xl rounded-tl-xl rounded-tr-xl bg-orange-50" : "place-self-start rounded-br-xl rounded-tr-xl rounded-tl-xl bg-stone-700 text-stone-100"}`} key={i + Chat.chat.length}>
            ...
            <footer className='w-screen h-[11vh] bg-stone-800 fixed bottom-0 left-0 flex justify-center items-center'>
                <input value={message} onKeyDown={handleSend} onChange={handleChange} className="p-3 rounded-bl-full rounded-tl-full bg-orange-50 outline-none text-sm w-[16rem]" type="text" placeholder='message' />
                <button disabled={isSend} onClick={handleSendClick} className={`text-stone-50 bg-blue-500 p-3 rounded-br-full rounded-tr-full ${isSend && "opacity-80"}`}>
